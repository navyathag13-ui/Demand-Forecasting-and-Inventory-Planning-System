...
# Store type -> integer code. Fixed convention, not fitted from data, but
# named and exported here so nothing downstream (serving included) hardcodes
# its own copy.
...
def trend_reference_date(df: pd.DataFrame) -> pd.Timestamp:
    """
    First date in the dataset -- the anchor `trend` (weeks-since-start) is
    measured from. Exposed so serving-time feature recomputation can anchor
    new events to the same reference the model was trained against, instead
    of re-deriving (and potentially drifting from) its own start date.
    """
    return df["Date"].min()
...
def add_time_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    min_date = trend_reference_date(df)
    df["trend"] = ((df["Date"] - min_date).dt.days / 7).astype(int)
    return df


def size_normalization_params(df: pd.DataFrame) -> tuple[float, float]:
    """
    Mean/std of store Size, fitted here once so training and serving-time
    feature recomputation normalise size_norm identically instead of two
    independently-computed (and potentially divergent) statistics.
    """
    return float(df["Size"].mean()), float(df["Size"].std())


def add_store_features(df: pd.DataFrame) -> pd.DataFrame:
    """Encode store type and normalise store size."""
    df = df.copy()
    df["type_code"]        = df["Type"].map(STORE_TYPE_MAP).astype(int)
    size_mean, size_std    = size_normalization_params(df)
